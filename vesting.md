...
- Vault status must be `Active`
- **Current timestamp must be before the start_timestamp**
...
This section outlines the security assumptions, trust model, and known limitations of the Disciplr Vault contract to assist auditors and users.
...
2. **Creator Power**: The `creator` is the only address authorized to `create_vault` or `cancel_vault`. They must authorize the initial USDC funding.
3. **Restricted Cancellation Window**: Vault cancellation is only permitted **before** the `start_timestamp`. Once the commitment period begins, funds are locked to ensure economic fairness and prevent creators from withdrawing funds after project work has likely commenced. This prevents unilateral withdrawal once the trust-based project phase has started.
4. **No Administrative Overrides**: There is no "admin" or "owner" role with the power to sweep funds or override the vault logic. Funds can only flow to the predefined `success_destination`, `failure_destination`, or back to the `creator` on cancellation (within the window).
...
| 0.1.1 | Restricted cancellation to pre-start window for economic fairness |
